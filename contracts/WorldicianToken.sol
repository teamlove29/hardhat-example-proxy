...
contract WorldicianToken is IWorldicianToken ,ERC721Checkpointable, Ownable, ReentrancyGuardUpgradeable{
    ...
    event UpdateMaxSupply(uint256 oldSupply, uint256 newSupply);
    ...
    constructor(address _minter, address _weth, address _adminDAO) ERC721('Worldicians', 'WORLDICIAN') {
        minter = _minter;
        weth = _weth;
        adminDAO = _adminDAO;
        maxSupply = 444;
        _currentWorldicianId = 1;
    }

    function mint() public override onlyMinter returns (uint256) {
        require(_currentWorldicianId <= maxSupply,"WorldicianToken: Max Supply!"); 
        if(_currentWorldicianId > 444){
    ...
    function setAdminDAO(address _adminDAO) external onlyOwner{
        adminDAO = _adminDAO;
    }
    ...
    function setMaxSupply(uint256 _newMaxSupply) external onlyOwner {
        uint256 _oldSupply = maxSupply;
        maxSupply = _newMaxSupply;
        emit UpdateMaxSupply(_oldSupply,_newMaxSupply);
    ...
    function tradeValid(address maker, address taker, uint256 makerWei, uint256[] memory makerIds, uint256 takerWei, uint256[] memory takerIds, uint256 expiry, uint256 salt, bytes memory signature) view public returns (bool) {
        ...
        for (uint i = 0; i < offer.makerIds.length; i++) {
            require(ownerOf(offer.makerIds[i]) == offer.maker, "At least one maker token doesn't belong to maker.");
        }
        ...
        if (offer.taker == address(0)) {
            ...
            for (uint i = 0; i < offer.takerIds.length; i++) {
                require(ownerOf(offer.takerIds[i]) == offer.taker, "At least one taker token doesn't belong to taker.");
            }
        }
        return true;
    }
    ...
    function _mintTo(address to, uint256 worldicianId) internal returns (uint256) {
        ...
        emit WorldicianCreated(worldicianId);
        return worldicianId;
    }
}